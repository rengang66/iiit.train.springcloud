...
public class ProductEndpoint {
     ...
     * @return
     */
    @RequestMapping(value = "/{id}/lbccomments", method = RequestMethod.GET)
    public List<ProductCommentDto> lbccomments(@PathVariable Long id){
        List<ProductComment> commentList = this.productCommentRepository.findByProductIdOrderByCreated(id);
        if (null == commentList || commentList.isEmpty())
            return Collections.emptyList();
    ...
    public UserDto loadUserEx(Long userId) {
        ...
        UserDto userDto = this.lbcRestTemplate.getForEntity(userUri.toString(), UserDto.class, userId).getBody();
        if (userDto != null) {
            this.logger.debug("I came from server : {}", userDto.getUserServicePort());
        }
        return userDto;
